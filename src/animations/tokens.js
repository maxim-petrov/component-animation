 ...
 * - Длительность: 100-500мс оптимальный диапазон
...
export const Duration = {
  XS: 0.1,    // 100ms - микро-анимации, незначительные изменения (ховер)
  S: 0.15,    // 150ms - небольшие UI-изменения (кнопки, переключатели)
  M: 0.25,    // 250ms - стандартные переходы (подсказки, модальные окна)
  L: 0.3,     // 300ms - крупные изменения (переходы между экранами)
...
export const Easing = {
  ...
  entrance: [0.0, 0.0, 0.3, 1.0],
  
  // Для исчезновения элементов - медленный старт, резкое ускорение к концу
  exit: [0.4, 0.14, 1.0, 1.0],
  
  // Пружинная динамика для эмоциональных эффектов
